...
public:
	...
	Block(int num) :
		blockNo(num), pages(new PhysicalPage[PAGES_PER_BLOCK]), valid(0), nextFree(
			0) {
		...
		*counter = 0;
		...
		for (int i = 0; i < PAGES_PER_BLOCK; i++) {
			if (pages[i].status == VALID) {
				...
				logicalPages[*counter]->physicalPage = nullptr;
				logicalPages[*counter]->status = FREE_LOGICAL;
	...
	int write(int lpn, char* data, LogicalPage* page) {
		PhysicalPage* current = &(pages[nextFree]);
		assert(current->blockNo < PHYSICAL_BLOCK_NUMBER);
		assert(lpn != NA);
		page->lpn = lpn;
...
public:
	...
	unsigned long long to_rewrite;
	unsigned long long rewriten;
	...
	explicit FTL() :
            mappingTable(new LogicalPage[LOGICAL_BLOCK_NUMBER * PAGES_PER_BLOCK]), 
			blocks(new Block*[PHYSICAL_BLOCK_NUMBER]),
			V(new set<int> [PAGES_PER_BLOCK + 1]), Y(0), erases(0), erases_steady(0), 
			logicalPageWrites(0), logicalPageWritesSteady(0), physicalPageWrites(0), physicalPageWritesSteady(0),
			to_rewrite(0), rewriten(0),
            print_mode(false) {
...
#undef X
	...
	void updateObsolete(Block* block) const {
		...
		if (block->nextFree == BLOCK_FULL) {
			...
			assert(valid < PAGES_PER_BLOCK + 1 && valid >= 0);
	...
	void copyValidToNewPlace(char* data, LogicalPage* logicalPages[],
		...
		for (int i = 0; i < counter; i++) {
			...
			result = current->write(logicalPages[i]->lpn, data + i * PAGE_SIZE, logicalPages[i]);
			physicalPageWrites++;
	...
	void blockCleanGenaretional(Block* block, list<LogicalPage**>& pages_to_rewrite) {
		char tempData[PAGES_PER_BLOCK * PAGE_SIZE];
		int counter;
		...
		logicalPageWrites -= counter;
		 to_rewrite += counter;
		for (int i = counter; i < PAGES_PER_BLOCK; i++) {
			logical_pages[i] = nullptr;
		}
		pages_to_rewrite.push_back(logical_pages);
	}
	...
	void GCWithLookAhead(unsigned int* writing_sequence, unsigned int base_index, bool rewrite_generational = false, list<LogicalPage**>* pages_to_rewrite = nullptr) {
        ...
        assert(!freeList.empty());
        V[min->valid].erase(min->blockNo);
		if (rewrite_generational) {
			blockCleanGenaretional(min, *pages_to_rewrite);
			assert(!(*pages_to_rewrite).empty());
		}
		else {
			blockClean(min);
		}
    ...
    void updateMappingTable(int lpn, Block* current) const{
		assert(current);
		assert(lpn < LOGICAL_BLOCK_NUMBER* PAGES_PER_BLOCK);
		assert(lpn != NA);
		assert(mappingTable[lpn].physicalPage);
		assert(mappingTable[lpn].physicalPage->blockNo >= 0 && 
			mappingTable[lpn].physicalPage->blockNo < PHYSICAL_BLOCK_NUMBER);
        Block *obsoletePlace =
	...
	void write(char* data, int lpn, Algorithm algorithm, unsigned int* writing_sequence = nullptr, unsigned long long base_index = NA) {
		if (freeList.empty()) {
			if (algorithm == GREEDY) {
				GC();
			}
        ...
        int result = current->write(lpn, data, &(mappingTable[lpn]));
        physicalPageWrites++;
	...
	void writeGenerational(char* data, int lpn, int generation, unsigned int* writing_sequence, unsigned long long base_index,
		bool generational_gc = false, list<LogicalPage**>* pages_to_rewrite = nullptr) {
		Block* gen_block = getGenerationalBlock(generation);
		if (!gen_block) {
			if (freeList.empty()) {
				GCWithLookAhead(writing_sequence, base_index, generational_gc, pages_to_rewrite);
				if (pages_to_rewrite && to_rewrite > rewriten) {
					assert(!(*pages_to_rewrite).empty());
		...
		assert(lpn != NA);
		if (mappingTable[lpn].status != FREE_LOGICAL && mappingTable[lpn].physicalPage) {
			updateMappingTable(lpn, gen_block);
		}
		int result = gen_block->write(lpn, data, &(mappingTable[lpn]));
		physicalPageWrites++;
		...
		void NewBlockClean(Block * block) {
			...
			for (int i = 0; i < counter; i++) {
				logicalPages[i]->clear();
				block->write(i, data, logicalPages[i]);
				physicalPageWrites++;
			}
		}
		...
		void writeToBlock(char* data, int lpn, int block_number) {
			...
			int result = write_to->write(lpn, data, &(mappingTable[lpn]));
			physicalPageWrites++;
